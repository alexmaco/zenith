...
impl CPUTimeApp {
    ...
    fn update_process_list(&mut self, keep_order: bool) {
        ...
        struct Top {
            ...
            read_pid: Option<i32>,
            write: f64,
            write_pid: Option<i32>,
        }
        impl Top {
            fn update(&mut self, zp: &ZProcess, tick_rate: &Duration) {
                if zp.cum_cpu_usage > self.cum_cpu_usage {
                    self.cpu_pid = Some(zp.pid);
                    self.cum_cpu_usage = zp.cum_cpu_usage;
                }
                if zp.memory > self.mem {
                    self.mem = zp.memory;
                    self.mem_pid = Some(zp.pid);
                }
                if zp.get_read_bytes_sec(tick_rate) > self.read {
                    self.read = zp.get_read_bytes_sec(tick_rate);
                    self.read_pid = Some(zp.pid);
                }
                if zp.get_write_bytes_sec(tick_rate) > self.write {
                    self.write = zp.get_write_bytes_sec(tick_rate);
                    self.write_pid = Some(zp.pid);
                }
            }
        }
        let mut top = Top::default();
        top.cum_cpu_usage = match &self.cum_cpu_process {
        ...
        self.top_mem_pid = top.mem_pid.or(self.top_mem_pid);
        self.top_disk_reader_pid = top.read_pid.or(self.top_disk_reader_pid);
        self.top_disk_writer_pid = top.write_pid.or(self.top_disk_writer_pid);
